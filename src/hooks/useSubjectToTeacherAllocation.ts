...
const useSubjectToTeacherAllocation = () => {
  ...
  const loadSubjetAllocationData = async () => {
    try {
      ...
      const subjectAllocationData =
        response_obj.response?.data.data.subject_to_teacher_map
      const teacherAllocationData =
        response_obj.response?.data.data.teacher_to_subject_map
      ...
      dispatch(
        updateSubjectAllocationState({
          ...
          isDataLoaded: true,
          status: null,
          SubjectToTeacherAllocation: subjectAllocationData,
          TeacherToSubjectAllocation: teacherAllocationData,
        }),
      )
    } catch (error: any) {
      toast.error(
  ...
  const handleOnChangeManageHoursForSubjectToTeacher = (
    current_hours: number,
    new_hours: number,
    subject_slug: string,
    teacher_slug: string,
    subject_type: string,
    allocation_catgory: any,
    flag: boolean,
  ) => {
    try {
      ...
      if (isIncreasing) {
        ...
        const allocation_type =
          teacher[allocation_catgory as keyof typeof teacher]

        if (
          Object.keys(allocation_catgory).length === 0 ||
        ...
        if (subject_type !== 'practical') {
          state.stats.unallocated_subject_hours_instant_allocation -=
            difference_hours
          state.stats.unallocated_teacher_hours_instant_allocation -=
            difference_hours
        } else {
          state.stats.unallocated_subject_hours_serparate_allocation -=
            difference_hours
          state.stats.unallocated_teacher_hours_separate_allocation -=
            difference_hours
        }
      } else {
        ...
        if (new_hours == 0) {
          ...
          const teacher_index = subject_allocation.teachers.findIndex(
            (d: any) => d.teacher.slug === teacher_map.teacher.slug,
          )
          if (teacher_index !== -1) {
  ...
  const handleOnClickToDeleteAllocation = (
    subject_slug: string,
    teacher_slug: string,
    subject_type: string,
    ...
    try {
      ...
      const subject_allocation =
        subject_map.teacher_allocation[
          allocation_category as keyof TeacherAllocation
        ]
      const teacher_map = subject_allocation.teachers.find(
        (d: any) => d.teacher.slug === teacher_slug,
      )
  ...
  const getTeacherList = (allocation_category: string) => {
    try {
      ...
      const hours =
        allocation_category == 'Lab' || allocation_category == 'Practical'
          ? 2
          : 1
      const filterdData = Object.entries(teacherAllocation[0]).filter(
        ([_, value]) => value.total_hours_left >= hours,
      )
  ...
  const addTeacherToSubject = (
    ...
    try {
      ...
      const subject_allocation =
        subject_map.teacher_allocation[
          allocation_category.toLowerCase() as keyof TeacherAllocation
        ]
      if (subject_allocation.remaining_hours - hours >= 0) {
        ...
        throw new Error(
          'please allocate the less number of hours to teacher as total remaining hours are :' +
            subject_allocation.remaining_hours,
        )
      }
    } catch (error: any) {
      toast.error(error.message || 'something went worng')
