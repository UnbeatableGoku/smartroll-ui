...
const useUnifiedSubjectChoiceForTeacher = () => {
    ...
    try {
      ...
      if (response_obj.response?.data.data.subject_choices.choices_saved) {
        ...
        sortedSubjects.forEach((subject: any) => {
          const already_subject = similar_subject.some((d: any) => d.slug === subject.slug)
          if (!already_subject) {
  ...
  const toggleSubjectSelection = (subject: any): void => {
    ...
    setSimilarSubjects((prevData: any) => {
      ...
      if (already_subject) {
        ...
        const filteredData = prevData.filter((d: any) => d.slug !== subject.slug);
        return filteredData;
      } else {
        if (subject.similar_subjects) {
          // remove similar subjects if they are not already in prevData
          const updatedSimilarSubjects = prevData.filter(
            (simData: any) => subject.similar_subjects.some((d: any) => d.slug !== simData.slug)
          );
          //add the similar subject
          const remainingSubject = subject.similar_subjects.filter(
            (simData: any) => !similarSubjects.some((d: any) => d.slug === simData.slug)
          );
  ...
  const togglePanel = () => {
    const final_subjects = savedSubjects
      .filter(
        (subject: any) =>
